...
namespace Photo.Net.Base
{
    ...
    public unsafe static class Memory
    {
        ...
        public static IntPtr Allocate(ulong bytes)
        {
            ...
            {
                IntPtr block = SafeNativeMethods.HeapAlloc(_hHeap, 0, new UIntPtr(bytes));
        ...
        {
            var bmi = new NativeStructs.BITMAPINFO
            {
                bmiHeader =
                {
                    biSize = (uint)sizeof(NativeStructs.BITMAPINFOHEADER),
                    biWidth = width,
                    biHeight = -height,
                    biPlanes = 1,
                    biBitCount = 32,
